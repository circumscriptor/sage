...
bool VirtualConsole::Initialize() {
    static Internal::FileIOCallbacksSDL sFileIO;
...
void VirtualConsole::LoadConfig() {
    std::string configFile = fmt::format("{}{}", Sage::IO::Path::User(), SAGE_CONFIG_FILE_NAME);
    ...
    if (auto* RWops = SDL_RWFromFile(configFile.c_str(), "r"); RWops == nullptr) {
        ...
        Sage::Console::VirtualConsole::Get()->SaveConfig(configFile);
    } else {
        SDL_RWclose(RWops);
    }

    Sage::Console::VirtualConsole::Get()->ReloadConfig(configFile);
}
